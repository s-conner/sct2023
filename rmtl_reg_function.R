...
# covariates <- dat[, c('TRT01PN'),drop=FALSE]
# times=dat$PFS; event=dat$cause; eoi=1; tau=24; cov=as.matrix(covariates); strata=FALSE

rmtl.reg <- function(times, event, eoi=1, tau, cov=NULL, strata=FALSE, group=NULL){
  ...
  # Overall quantities
  x <- cbind(Intercept=rep(1, length(y)), cov)
  p <- length(x[1,])
  ...
  if(strata==TRUE){
    ...
    # Need to assign weights according to original ID, not ordered by event time
    linked.weights.a <- cbind(orig.id.a, weights.a, delta1.a, d0.a, y.a)
    weights <- rbind(weights, linked.weights.a)
  }
  ...
  # lm.fit <- lm(tau-y ~ x-1, weights=weights, data=data)
