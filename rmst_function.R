...
rmst <- function(time, status, group, weight=NULL, tau=NULL, alpha=.05, plot=TRUE, xaxismin=0, xaxismax=max(time), ...){
  if(sum(time<0)>0){print("Error: times must be positive.")
    ...
    if(sum(weight<=0)>0){print("Error: weights must be greater than 0.")
      ...
      if(sum(status!=0 & status!=1)>0){print("Error: status must be a vector of 0s and/or 1s.")
        ...
        if(plot==TRUE){plot(NULL, xlim=c(xaxismin, xaxismax), ylim=c(0,1), xlab='Time',ylab='Survival Probability')}
        ...
        for (i in 1:j){
          ...
          #--- Plot AKM ---
          if(plot==TRUE){lines(ft$tj, ft$st,type="s", col=(i+2), lwd=2)}
        }
      }
    }
  }
  ...
  if(plot==TRUE){abline(v=tau, col=1, lty=3, lwd=2)
  legend('bottomleft', paste(groupval), lty=rep(1, j), lwd=rep(2, j), col=3:(j+2), 
    ...
    if(sum(time<0)>0){print("Error: times must be positive.")
      ...
      if(sum(weight<=0)>0){print("Error: weights must be greater than 0.")
        ...
        if(sum(status!=0 & status!=1)>0){print("Error: status must be a vector of 0s and/or 1s.")
          ...
          j=length(unique(data$group))
          groupval <- rep(999, length(1:j))
          plot(NULL, xlim=c(xaxismin, xaxismax), ylim=c(0,1), xlab='Time',ylab='Survival Probability')
