...
rmtl <- function(times, event, eoi=1, group=NULL, weight=NULL, tau=NULL, alpha=.05, entry=NULL, plot=TRUE, ...){  
  ...
  if(sum(times<0)>0){print("Error: times must be positive.")
    ...
    if(sum(tau.error)>0){
      ...
      if(plot==TRUE){plot(NULL, xlim=c(0, tau), ylim=c(0,1), xlab='Time', ylab='Cumulative incidence')}
      ...
      for (g in 1:gg){
        ...
        theta <- s * h1
        cif1 <- cumsum(theta)
        if(plot==TRUE){lines(c(tj, tau), c(cif1, cif1[num.tj]), type="s", col=(g+2), lwd=2)}
